...
if (window.addEventListener) {
 ...
 function DisableDrag(){
        ...
        REDIPS.drag.enableDrag(false, '.redips-drag');
        
      });
}

function MarkBookedTables(table_bookings){

    $.each(table_bookings, function( index, value ) {
        
        var table_no = value['table_no'];
        
//        $('tbody').find('div:contains('+table_no+')').removeClass("notreserved");
        
        $('tbody').find('div:contains('+table_no+')').filter(function() {
            return $(this).text() === table_no.toString();
                }).addClass("reserved").removeClass("notreserved");
         

        $('tbody').find('.reserved').append('<br>reserved');
        $('tbody').find('.reserved').css("background-color", "red");
        $('tbody').find('.reserved').css("overflow", "hidden");
        $('tbody').find('.reserved').parent().find('#theImg').css("height", "60px");
        
    });
}

function MarkOneTableAsBooked(booked_table){

        var table_no = booked_table;
