...
//					div[i].style.filter = 'alpha(opacity=' + opacity + ')';					
	...
	loadContentHandler = function (xhr, obj) {
        ...
        // iamges attaching to cells method
        var attachImagesToCells = function (no_of_images) {
            for(i = 1; i <= no_of_images; i++){
//                $('.table_cr'+i).prepend($('<img>',{id:'theImg',src:'/imgs/tables_types/table_cr'+i+'.jpg', height:'70', width:'70'}));
	...
	insertContent = function (targetTable, param) {
		...
		for (i = 0; i < json.length; i++) {
			...
			text = json[i][4];	// DIV text
                        cloned_items = json[i][4];
//                        console.log(cloned_items);
                        if(cloned_items > counter) { counter = cloned_items; }
			// create DIV element
			div = document.createElement('div');
			// set DIV id
			div.id = id;
			// set class names with normalized spaces to the cloned DIV element
			div.className = normalize('redips-drag notreserved ' + className);
			div.textContent = text;
//                        div.prepend("<img src='/imgs/tables_types/"+className+".jpg'/>");
//                        $('#'+id+'').prepend('<img src="/imgs/tables_types/table_cr4.jpg" />');
			...
			// target row and target cell exist -> append and enable DIV element in TD
                ...
                // iamges attaching to cells
//                attachImagesToCells(13);
                for(i = 1; i <= 13; i++){
                $('.table_cr'+i).prepend($('<img>',{id:'theImg',src: $('.imgs_url').val()+'/tables_types/table_cr'+i+'.jpg', height:'70', width:'70'}));
            } 
